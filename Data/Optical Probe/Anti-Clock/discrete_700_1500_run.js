...
var minVal = 1400;       // Min. input value [700us, 2300us] 
var maxVal = 2100;       // Max. input value [700us, 2300us]
var stepsQty = 120;        // Number of steps
var settlingTime = 10;    // Settling time before measurement input change [s]
var samplesAvg = 10;     // Number of samples to average
var stepsGoDown = true;  // If true, the test will step down, if false, the steps will only go up.
var repeat = 4;          // Number of times to repeat the same sequence
var filePrefix = "StepsTest_anticlock_run1";
...
//ESC initialization
rcb.console.print("Initializing Servo...");
...
function startSteps(){
    ...
    rcb.output.steps("servo3", minVal, maxVal, stepsQty, stepFct);
}
        ...
        rcb.files.newLogEntry(result,function(){
            if(isLastStep){ // If it is the last step
                if (goingUp && stepsGoDown){
                    ...
                    rcb.console.setVerbose(true);
                    rcb.output.steps("servo3", maxVal, minVal, stepsQty, stepFct);
                }else{
                    if(repeat>1){
                        repeat --;
